...
export function main(): void {
  ...
  let svg = document.querySelector("svg") as Element;
  if (svg) graph.setOutput(svg).initSvgGraph();
...
function initInputHandlers(graph: Graph): void {
  graph.idealNumNodes = Math.round(50);
  graph.extraEdgeProportion = 20 / 100;
  graph.radiiWeightPower = 0.01;
...
class Graph {
  ...
  private updateNodes(): void {
    ...
    let curIdealNumNodes = Math.min(
      Math.floor(this.frameNumber / 15),
      this.idealNumNodes
    );
    ...
    while (newNodes.length < curIdealNumNodes) {
      newNodes.push(
        new GNode(
          ...
          (Math.pow(Math.random(), 5) + 0.35) * 0.005, // Radius skewing toward smaller values
          0.0,
          0.0
        )
      ); // Velocity
  ...
  private calcSpanningTree(
    allEdges: Readonly<Array<[number, number, number]>>
  ): Array<GEdge> {
  ...
  private static containsEdge(
    edges: Readonly<Array<GEdge>>,
    edge: GEdge
  ): boolean {
      ...
      for (const node of this.nodes) {
        gElem.append(
          createSvgElem("circle", {
            ...
            cy: node.posY,
            r: node.radius,
            fill: "rgba(256,256,256," + node.opacity.toFixed(3) + ")",
          })
        );
      }
        ...
        if (mag > a.radius + b.radius) {
          ...
          gElem.append(
            createSvgElem("line", {
              ...
              stroke: "rgba(256,256,256," + opacity.toFixed(3) + ")",
            })
          );
        }
      }
...
class GObject {
  ...
  public fade(delta: number): void {
    this.opacity = Math.max(Math.min(this.opacity + delta, 0.1), 0.0);
  }
}
